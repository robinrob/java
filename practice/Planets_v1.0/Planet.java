...
//-----------------------------------80 characters------------------------------     
     ...
     */
    public void reCalculate(ArrayList<Planet> planets)
    {
        ...
        for (Planet planet : planets){
            if (!this.equals(planet)){
                double xSep = planet.getXPos() - xPos;
                double ySep = planet.getYPos() - yPos;
               // if (Math.sqrt((xSep*xSep + ySep*ySep)) < (diameter + planet.getDiameter())){
                    //destroyed = true;
                //}
                double rSep = Math.sqrt(xSep*xSep + ySep*ySep);
                accelerationX += G*(planet.getMass()/rSep*rSep)*(xSep/rSep);
                accelerationY += G*(planet.getMass()/rSep*rSep)*(ySep/rSep);
    ...
    public void move()
    {
        if((xPos + xSpeed) > (xLimit - 1)){
            xPos += xSpeed - (xLimit - 1);
        }
        else{
            xPos += xSpeed;
        }
        
        if((yPos + ySpeed) > (yLimit - 1)){
            yPos += ySpeed - (yLimit - 1);
        }
        else{
            yPos += ySpeed;
        }
